...
def load_music():
    ...
    try:
        if not music_folder.exists():
            ...
            return None
        music_files = [path for path in music_folder.iterdir() if path.suffix.lower() in {".mp3", ".wav", ".ogg"}]
        if not music_files:
...
def create_coin_sound():
    ...
    for index in range(sample_count):
        ...
        sample = int(math.sin((2.0 * math.pi * pitch * index) / sample_rate) * max_amplitude * envelope)
